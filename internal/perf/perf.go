...
func (pr *perfRunner) createMsgConfirmSub(nodeURL, name, tag string) (subID string, subName string, err error) {
	...
	fullPath, err := url.JoinPath(nodeURL, pr.cfg.APIPrefix, "api/FireflyService", pr.cfg.FFNamespace, "subscriptions")
	if err != nil {
		return "", "", err
	}
	_, err = pr.client.R().
...
func (pr *perfRunner) createEthereumContractListener(nodeURL string) (string, error) {
	...
	fullPath, err := url.JoinPath(nodeURL, pr.cfg.APIPrefix, "api/FireflyService", pr.cfg.FFNamespace, "contracts/listeners")
	if err != nil {
		return "", err
	}
	res, err := pr.client.R().
...
func (pr *perfRunner) deleteSubscription(nodeURL string, subscriptionID string) error {
	fullPath, err := url.JoinPath(nodeURL, pr.cfg.APIPrefix, "api/FireflyService", pr.cfg.FFNamespace, "subscriptions", subscriptionID)
	if err != nil {
		return err
	}
	_, err = pr.client.R().
...
func (pr *perfRunner) deleteContractListener(nodeURL string, listenerID string) error {
	fullPath, err := url.JoinPath(nodeURL, pr.cfg.APIPrefix, "api/FireflyService", pr.cfg.FFNamespace, "contracts/listeners", listenerID)
	_, err = pr.client.R().
		SetHeaders(map[string]string{
			"Accept": "application/json",
		}).
...
func (pr *perfRunner) getMintRecipientBalance() (int, error) {
	...
	fullPath, err := url.JoinPath(pr.client.BaseURL, pr.cfg.APIPrefix, "api/FireflyService", pr.cfg.FFNamespace, "tokens/balances")
	if err != nil {
		return 0, nil
	}
	res, err := pr.client.R().
